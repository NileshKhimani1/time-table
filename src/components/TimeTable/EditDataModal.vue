<template>
  ...
  <div class="modal">
    ...
    <input type="text" v-model="link" id="link" placeholder="Enter the link" />
    <label for="time">Pick the Time : </label>
    <vue-time-picker v-model="time"></vue-time-picker>
    <input
      type="submit"
      @click.prevent="
        $emit('edit-data-entry', { code, name, venue: link, time })
      "
      value="Edit Data"
    />
  </div>
...
export default {
  emits: ["close-modal", "edit-data-entry"],
  props: ["currentData"],
